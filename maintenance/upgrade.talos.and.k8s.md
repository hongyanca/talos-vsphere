...
```bash
# Replace Talos version from 1.13.x to 1.13.4 on Linux
sed -i 's/:v1\.13\.[0-9]\+/:v1.13.4/g' controlplane*.yaml worker*.yaml
sed -i 's/v1\.13\.[0-9]\+/v1.13.4/g' vmware.sh vmware-deploy-node.sh

# Replace Talos version from 1.13.x to 1.13.4 on macOS
# Use `brew install gnu-sed` to install GNU sed
gsed -i 's/:v1\.13\.[0-9]\+/:v1.13.4/g' controlplane*.yaml worker*.yaml
gsed -i 's/v1\.13\.[0-9]\+/v1.13.4/g' vmware.sh vmware-deploy-node.sh
